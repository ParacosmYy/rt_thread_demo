...
static rt_mutex_t test_tex = RT_NULL;  // 定义互斥锁
...
int main(void)
{	
    rt_kprintf("这是一个[野火]- STM32 全系列开发板 RTT 互斥锁同步实验！\n");
    rt_kprintf("同步成功则输出 Successful,反之输出 Fail\n");
   
    test_tex = rt_mutex_create("test_tex", RT_IPC_FLAG_PRIO);//创建互斥锁
    if (test_tex == RT_NULL)
    {
        rt_kprintf("test_tex creation failed!\n");
        return -1;
    }
    /* 创建发送线程 */
...
static void send_thread_entry(void* parameter)
{
  while(1) {
    ...
    value[1]++; // 更新第二个值
    rt_mutex_release(test_tex); // 发送完成信号
...
{
  while(1) {
    rt_mutex_take(test_tex, RT_WAITING_FOREVER); // 等待信号
    if(value[0] == value[1]) {
